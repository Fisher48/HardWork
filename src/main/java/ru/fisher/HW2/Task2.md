...
Еще нашел ошибку UnsupportedOperationException при передаче таймзоны клиента для получения поездки.
Там передается пустая тайзона и вроде везде есть дополнительные проверки, но ошибка не исчезла, буду еще пытаться её устранить.


В любом случае это хороший опыт и знание, который будем полезным.
