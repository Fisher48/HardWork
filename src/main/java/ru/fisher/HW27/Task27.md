...
Попытка вызвать недопустимые методы просто не пройдет.

Второе - это то что каждый класс-состояние отвечает только за действия в одном конкретном состоянии.
Состояние определяется через тип объекта, а не через внутренний enum или булевы флаги. Чтение кода становится проще.

Третье - повышение безопасности и предотвращение ошибок.
Терминальные состояния вообще не имеют методов перехода — любая попытка изменить финальное состояние является ошибкой компиляции. 
В оригинальном коде те же проверки выполнялись например, через `isValidTransition()`, `canBeCancelled()` и т.д., 
а ошибки обнаруживались только на проде через исключения.
